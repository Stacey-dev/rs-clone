...
export function createCalendar(container: HTMLDivElement, year: number, month: number, lng: string) {
    const d = new Date(year, month);
    const saturdays = getAllSaturdays(d);

    let table = '<table>';

    container.innerHTML = createCalendarsTable(table, d, month)
